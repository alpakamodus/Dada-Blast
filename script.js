...
canvas.addEventListener("pointerdown", (e) => {
    blocks.forEach(b => {
        if (e.clientX >= b.x - blockSize / 2 && e.clientX <= b.x + blockSize / 2 && e.clientY >= b.y - blockSize / 2 && e.clientY <= b.y + blockSize / 2 && !b.placed) {
            b.x = e.clientX;
            b.y = e.clientY;
        }
    });
...
function checkForSnap(b) {
    for (let y = 0; y < 8; y++) {
        for (let x = 0; x < 8; x++) {
            if (Math.abs(b.x - (boardX + (x * blockSize + blockSize / 2))) <= blockSize / 2.1 && Math.abs(b.y - (boardY + (y * blockSize + blockSize / 2))) <= blockSize / 2.1 && !grid[y][x]) {
                b.x = b.homeX;
                b.y = b.homeY;
                grid[y][x] = true;
                placeCount++;
